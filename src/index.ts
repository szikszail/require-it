...
const getGlobalRoot = (): string => {
    if (!globalRoot) {
        globalRoot = execSync("npm root -g", { encoding: "utf8" }).trim().replace(/node_modules$/, "");
    }
    return globalRoot;
};

const checkScopedNodeModulesOfFolder = (folder: string, name: string): string => {
...
const checkNodeModulesOfFolder = (folder: string, name: string): string => {
    ...
    for (const nodeModule of nodeModules) {
        ...
        getNodeModulesOfFolder(join(root, nodeModule, "node_modules")).forEach((subModule: string): void => {
...
const directory: ResolveFromFunction = (name: string, root?: string): string => {
...
requireGlobal.resolve = (name: string): string => resolve(name, getGlobalRoot());
requireGlobal.directory = (name: string): string => directory(name, getGlobalRoot());

export const requireFrom: RequireFromObject = ((module: string, root?: string): object => {
